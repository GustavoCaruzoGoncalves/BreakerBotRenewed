...
import type { ExtraHandler } from '../../lib/chatai.js';
...
const quizHandler: ExtraHandler = async (sock, msg, callAPI) => {
  ...
  const response = await callAPI([
    {
      role: 'system',
      content:
        'Você é um assistente que gera perguntas sobre coisas aleatórias do mundo com alternativas (A, B, C, D). Você começa com Pergunta: (pergunta), em baixo as alternativas e a resposta correta',
    },
...
const zhipuCommand: Command = {
  meta: {
    category: 'IA',
    entries: [
      {
        trigger: '!zhipu',
        description: 'Conversa com o GLM-4.5 mantendo o contexto do chat',
        usages: [{ syntax: '!zhipu <pergunta>', description: 'Pergunta em texto ou sobre imagem' }],
      },
      { trigger: '!resetZhipu', description: 'Limpa o contexto acumulado do Zhipu neste chat' },
      { trigger: '!quiz', description: 'Gera 10 perguntas de múltipla escolha com gabarito' },
    ],
  },
  handle,
};

export default zhipuCommand;
