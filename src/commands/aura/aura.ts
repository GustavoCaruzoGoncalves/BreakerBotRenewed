...
interface MogPending {
  fromKey: string;
  toKey: string;
}

interface MognowState {
  attackerKey: string;
  targetKey: string;
  gameStartsAt: number;
  gameEndsAt: number;
  countAttacker: number;
  countTarget: number;
}

const mogDuels = new Map<string, MogDuel>();
const mogPending = new Map<string, MogPending[]>();
const mognowActive = new Map<string, MognowState>();
...
async function downloadSticker(sock: WASocket, media: MediaRef | null): Promise<Buffer | null> {
  if (media?.type !== 'stickerMessage') return null;
  return downloadMedia(sock, media);
}

function getMentionedJid(raw: WAMessage): string | null {
  const jids = raw.message?.extendedTextMessage?.contextInfo?.mentionedJid;
  return Array.isArray(jids) && jids.length > 0 ? (jids[0] ?? null) : null;
}
...
async function checkNegativePunish(
  sock: WASocket,
  jid: string,
  userId: string,
): Promise<void> {
...
async function endMogDuel(sock: WASocket, chatId: string, duel: MogDuel): Promise<void> {
  ...
  if (!winnerKey) {
    await sock.sendMessage(chatId, {
      text: `⏱ Empate! (${countFrom} x ${countTo} mensagens) Ninguém ganha aura.`,
    });
    return;
  }
  ...
  const missionReward = (await aura.hasMission(winnerKey, 'duel_win'))
    ? await aura.completeMission(winnerKey, 'duel_win')
    : 0;
...
const handle: CommandHandler = async (sock, msg) => {
  ...
  if (event && lower === event.command.toLowerCase() && Date.now() <= event.endsAt) {
    if (event.type === 'first') {
      if (event.winnerKey) {
        await sock.sendMessage(
          jid,
          { text: '⏳ Esse evento já foi conquistado por alguém!' },
          { quoted: raw },
        );
        return true;
      }
    ...
    if (event.type === 'all') {
      const participants = event.participants ?? new Set<string>();
      if (participants.has(sender)) {
        await sock.sendMessage(
          jid,
          { text: '✅ Você já participou deste evento!' },
          { quoted: raw },
        );
        return true;
      }
  ...
  if (lower === '!mog aceitar') {
    ...
    const pending = list.find((p) => p.toKey === sender);
    if (!pending) {
      await sock.sendMessage(
        jid,
        { text: '⚠️ Não há desafio de duelo para você aceitar aqui.' },
        { quoted: raw },
      );
      return true;
    }
    ...
    mogDuels.set(jid, {
      ...
      countFrom: 0,
      countTo: 0,
      endTime: Date.now() + MOG_DURATION_MS,
    });
    await sock.sendMessage(jid, {
      text: '⚔️ Duelo começando! Mandem mensagens por *15 segundos*. Quem tiver mais mensagens ganha!',
    });
    setTimeout(() => {
      const d = mogDuels.get(jid);
      if (d) endMogDuel(sock, jid, d).catch(() => {});
    }, MOG_DURATION_MS + 500);
    return true;
  }
  ...
  if (lower.startsWith('!mog ') && !lower.startsWith('!mognow')) {
    ...
    if (sender === toKey) {
      await sock.sendMessage(
        jid,
        { text: '⚠️ Você não pode mogar a si mesmo.' },
        { quoted: raw },
      );
      return true;
    }
  ...
  if (lower.startsWith('!mognow')) {
    ...
    if (mognowActive.has(jid)) {
      await sock.sendMessage(
        jid,
        { text: '⚠️ Já há um ataque em andamento neste chat.' },
        { quoted: raw },
      );
      return true;
    }
    if (sender === targetKey) {
      await sock.sendMessage(
        jid,
        { text: '⚠️ Você não pode atacar a si mesmo.' },
        { quoted: raw },
      );
      return true;
    }
    ...
    for (let i = MOGNOW_COUNTDOWN; i >= 1; i--) {
      setTimeout(
        () => {
          sock.sendMessage(jid, { text: `${i}` }).catch(() => {});
        },
        (MOGNOW_COUNTDOWN - i) * 1000,
      );
    }
      ...
      () => {
        void (async () => {
          ...
          if (countTarget > countAttacker) {
            ...
            const missionReward = (await aura.hasMission(state.targetKey, 'survive_attack'))
              ? await aura.completeMission(state.targetKey, 'survive_attack')
              : 0;
            const total = 500 + missionReward;
            sock
              .sendMessage(jid, {
            ...
            sock
              .sendMessage(jid, {
                text: `⏱ Empate! (${countAttacker} x ${countTarget}) Ninguém ganha aura.`,
              })
              .catch(() => {});
          }
        })();
  ...
  if (lower === '!meditar') {
    ...
    const gained = options[Math.floor(Math.random() * options.length)] ?? 0;
    ...
    await sock.sendMessage(
      jid,
      {
        text:
          gained > 0
            ? `🧘 Meditação concluída. Você absorveu *+${aura.formatAura(gained)}* de aura. Total: *${aura.formatAura(total)}*`
            : `🧘 Meditação concluída. Sua aura permanece estável. Total: *${aura.formatAura(total)}*`,
      },
      { quoted: raw },
    );
    return true;
  ...
  if (lower === '!treinar') {
    ...
    const lastAt = Number(await aura.getCooldown(sender, 'lastTreinarAt')) || 0;
    ...
    if (lastAt && elapsed < COOLDOWN) {
      ...
      await sock.sendMessage(
        jid,
        {
          text: `⏳ Aguarde *${minLeft === 1 ? '1 minuto' : `${minLeft} minutos`}* para treinar de novo.`,
        },
        { quoted: raw },
      );
      return true;
    ...
    if (Math.random() < 0.5) {
      ...
      await sock.sendMessage(
        jid,
        {
          text: `💪 Treino intenso! *+${aura.formatAura(500)}* de aura. Total: *${aura.formatAura(total)}*`,
        },
        { quoted: raw },
      );
    } else {
      ...
      await sock.sendMessage(
        jid,
        {
          text: `💔 O treino foi além do limite. *-${aura.formatAura(1000)}* de aura. Total: *${aura.formatAura(total)}*`,
        },
        { quoted: raw },
      );
  ...
  if (lower === '!dominar') {
    ...
    const lastAt = Number(await aura.getCooldown(sender, 'lastDominarAt')) || 0;
    const elapsed = Date.now() - lastAt;
    if (lastAt && elapsed < COOLDOWN) {
      const hoursLeft = ((COOLDOWN - elapsed) / 3600000).toFixed(1);
      await sock.sendMessage(
        jid,
        { text: `⏳ Dominação disponível em *${hoursLeft}h*.` },
        { quoted: raw },
      );
      return true;
    }
    ...
    if (Math.random() < 0.5) {
      ...
      await sock.sendMessage(
        jid,
        {
          text: `👑 Dominação absoluta! *+${aura.formatAura(1000)}* de aura. Total: *${aura.formatAura(total)}*`,
        },
        { quoted: raw },
      );
    } else {
      await sock.sendMessage(
        jid,
        { text: '😤 A dominação falhou. Nenhuma aura obtida.' },
        { quoted: raw },
      );
    }
    return true;
  ...
  if (lower === '!ritual') {
    ...
    if (lastDate === today) {
      await sock.sendMessage(
        jid,
        { text: '⏳ O ritual só pode ser feito *uma vez por dia*. Volte amanhã.' },
        { quoted: raw },
      );
      return true;
    }
    ...
    lines.forEach((line, i) =>
      setTimeout(() => {
        sock.sendMessage(jid, { text: line }).catch(() => {});
      }, i * 1800),
    );
      ...
      () => {
        void (async () => {
          ...
          if (won) {
            sock
              .sendMessage(jid, {
                text: `👑💀 *O ritual te abençoou.* +${aura.formatAura(5000)} de aura. Total: *${aura.formatAura(total)}* 🔥`,
              })
              .catch(() => {});
          } else {
            sock
              .sendMessage(jid, {
                text: `💀🔥 *O ritual te consumiu.* -${aura.formatAura(5000)} de aura. Total: *${aura.formatAura(total)}* 💀`,
              })
              .catch(() => {});
  ...
  if (lower.startsWith('!respeito ')) {
    ...
    if (targetId === sender) {
      await sock.sendMessage(
        jid,
        { text: '⚠️ Você não pode dar respeito a si mesmo.' },
        { quoted: raw },
      );
      return true;
    }
    ...
    if (!result.ok) {
      await sock.sendMessage(
        jid,
        {
          text: `❌ Você precisa de pelo menos *${aura.formatAura(50)}* de aura para usar !respeito.`,
        },
        { quoted: raw },
      );
      return true;
  ...
  if (lower.startsWith('!elogiar ')) {
    ...
    if (targetId === sender) {
      await sock.sendMessage(
        jid,
        { text: '⚠️ Você não pode elogiar a si mesmo.' },
        { quoted: raw },
      );
      return true;
    }
    ...
    const [sM, tM] = await Promise.all([
      ...
      mentions.processSingleMention(targetId),
    ]);
    await sock.sendMessage(
      jid,
      {
  ...
  if (lower.startsWith('!provocar ')) {
    ...
    if (targetId === sender) {
      await sock.sendMessage(
        jid,
        { text: '⚠️ Você não pode provocar a si mesmo.' },
        { quoted: raw },
      );
      return true;
    }
  ...
  if (lower.startsWith('!elogiados')) {
    ...
    if (unique.length === 0) {
      ...
      await sock.sendMessage(
        ...
        {
          text: isMe
            ? '📋 Ninguém te elogiou ainda. Use *!elogiar @alguém* para elogiar e dar +100 de aura!'
            : `📋 Ninguém elogiou ${name} ainda.`,
          mentions: m?.mentions.length ? m.mentions : undefined,
        },
        { quoted: raw },
      );
    ...
    const prefix = isMe
      ? 'Quem te elogiou'
      : `Quem elogiou ${(await mentions.processSingleMention(targetId)).mentionText}`;
    await sock.sendMessage(
      jid,
      {
  ...
  if (lower.startsWith('!aura doar ')) {
    ...
    if (!amount || amount < 1) {
      await sock.sendMessage(
        jid,
        { text: '⚠️ Informe um valor válido. Ex: *!aura doar 100 @usuario*' },
        { quoted: raw },
      );
      return true;
    }
    if (targetId === sender) {
      await sock.sendMessage(
        jid,
        { text: '⚠️ Você não pode doar aura para si mesmo.' },
        { quoted: raw },
      );
      return true;
    }
    ...
    if (balance < amount) {
      await sock.sendMessage(
        jid,
        {
          text: `❌ Você precisa de pelo menos *${aura.formatAura(amount)}* de aura para doar. Seu saldo: *${aura.formatAura(balance)}*`,
        },
        { quoted: raw },
      );
      return true;
    ...
    if (!result.ok) {
      await sock.sendMessage(jid, { text: '❌ Erro ao transferir aura.' }, { quoted: raw });
      return true;
    }
  ...
  if (lower.startsWith('!aura farmar ')) {
    ...
    if (targetId === sender) {
      await sock.sendMessage(
        jid,
        { text: '⚠️ Você não pode farmar aura de si mesmo.' },
        { quoted: raw },
      );
      return true;
    }
    if (Math.random() < 0.5) {
      ...
      await sock.sendMessage(
        jid,
        {
          text: `💔 Falhou! Você perdeu *${aura.formatAura(200)}* de aura. Total: *${aura.formatAura(total)}*`,
        },
        { quoted: raw },
      );
      await checkNegativePunish(sock, jid, sender);
  ...
  if (lower.startsWith('!aura figurinha')) {
    ...
    if (!hash) {
      await sock.sendMessage(
        jid,
        { text: '❌ Não foi possível obter o hash desta figurinha.' },
        { quoted: raw },
      );
      return true;
    }
    try {
      const buffer = await downloadSticker(sock, media);
    ...
    await sock.sendMessage(
      jid,
      {
        text: '✅ Figurinha de aura definida! Use essa figurinha para ter chance de ganhar +100 de aura.',
      },
      { quoted: raw },
    );
    return true;
  ...
  if (lower.startsWith('!aura personagem')) {
    const match =
      text.match(/!aura\s+personagem\s+"([^"]+)"/i) || text.match(/!aura\s+personagem\s+(.+)/i);
    ...
    await sock.sendMessage(
      jid,
      { text: `✅ Personagem definido: *${character}*` },
      { quoted: raw },
    );
    return true;
  }
  ...
  if (/^!aura\s+miss[oõ]es\s*$/i.test(text.trim())) {
    ...
    if (!data) {
      await sock.sendMessage(
        jid,
        { text: '❌ Você ainda não tem dados de aura.' },
        { quoted: raw },
      );
      return true;
    }
  ...
  if (/^!aura\s+rank(ing)?\s*$/i.test(text.trim())) {
    ...
    if (ranking.length === 0) {
      await sock.sendMessage(
        jid,
        { text: '📈 Ninguém no ranking de aura ainda.' },
        { quoted: raw },
      );
      return true;
    }
    ...
    await sock.sendMessage(
      jid,
      { text: t, mentions: allMentions.length ? allMentions : undefined },
      { quoted: raw },
    );
    return true;
  }
  ...
  if (lower.startsWith('!aura info')) {
    ...
    if (isMe) {
      ...
      if (!targetId) {
        await sock.sendMessage(
          jid,
          { text: '⚠️ Use *!aura info me* ou *!aura info @usuario*' },
          { quoted: raw },
        );
        return true;
      }
    ...
    if (!data) {
      await sock.sendMessage(
        jid,
        { text: '❌ Esse usuário ainda não tem dados de aura no sistema.' },
        { quoted: raw },
      );
      return true;
    }
    ...
    await sock.sendMessage(
      jid,
      { text: t, mentions: m.mentions.length ? m.mentions : undefined },
      { quoted: raw },
    );
    return true;
  }
  ...
  if (
    lower === '!aura' ||
    (lower.startsWith('!aura ') &&
      !['figurinha', 'personagem', 'missoes', 'missões', 'farmar', 'doar', 'ranking', 'rank', 'info'].some(
        (k) => lower.includes(k),
      ))
  ) {
...
export async function processAuraSticker(sock: WASocket, msg: BotMessage): Promise<void> {
  const { jid, raw } = msg;
  ...
  if (!data.stickerDataUrl) {
    try {
      const buffer = await downloadSticker(sock, msg.media);
      if (buffer) {
        await aura.setStickerData(sender, hash, `data:image/webp;base64,${buffer.toString('base64')}`);
      }
    } catch {
      /* falha ao baixar a figurinha não impede o ganho de aura */
    }
  }
  ...
  await sock.sendMessage(
    jid,
    { text: `✨ +${aura.formatAura(100)} de aura! Total: *${aura.formatAura(newTotal)}*` },
    { quoted: raw },
  );
}

export async function processAuraMissions(sock: WASocket, msg: BotMessage): Promise<void> {
  const { jid, raw } = msg;
  if (raw.key.fromMe) return;
  ...
  if (await aura.hasMission(sender, 'messages_500')) {
    ...
    if (result) {
      await sock.sendMessage(
        jid,
        {
          text: `📬 Missão "Mande 50 mensagens" concluída! *+${aura.formatAura(result.reward)}* aura.`,
        },
        { quoted: raw },
      );
    }
  ...
  const type = Object.keys(raw.message ?? {})[0] ?? '';
  const isMedia = ['stickerMessage', 'imageMessage', 'videoMessage', 'documentMessage'].includes(
    type,
  );
  if (isMedia && (await aura.hasMission(sender, 'send_media'))) {
    ...
    if (result) {
      await sock.sendMessage(
        jid,
        { text: `📎 Missão "Envie mídia" concluída! *+${aura.formatAura(result.reward)}* aura.` },
        { quoted: raw },
      );
    }
  }
...
export async function handleAuraReaction(sock: WASocket, item: ReactionEvent): Promise<void> {
  ...
  if (result) {
    await sock
      .sendMessage(chatId, {
        text: `💀 Missão "Reaja 20x com 💀 ou ☠️" concluída! *+${aura.formatAura(result.reward)}* aura.`,
      })
      .catch(() => {});
  }
}

const auraCommand: Command = {
  meta: {
    category: 'Aura',
    entries: [
      {
        trigger: '!aura',
        description: 'Guia da aura e ações sobre o seu perfil',
        usages: [
          { syntax: '!aura info me', description: 'Seu perfil completo de aura' },
          { syntax: '!aura ranking', description: 'Ranking de aura do servidor' },
          { syntax: '!aura missoes', description: 'Suas missões diárias' },
          { syntax: '!aura doar <valor> @usuario', description: 'Transfere aura para alguém' },
          { syntax: '!aura farmar <quantidade>', description: 'Arrisca aura para tentar ganhar mais' },
          {
            syntax: '!aura figurinha',
            description: 'Define sua figurinha da sorte (envie ou responda uma)',
          },
          { syntax: '!aura personagem "nome"', description: 'Define o seu personagem' },
        ],
      },
      { trigger: '!meditar', description: 'Ganho diário de aura pela meditação' },
      { trigger: '!treinar', description: 'Ganho diário de aura pelo treino' },
      { trigger: '!dominar', description: 'Tentativa arriscada de dominar e ganhar muita aura' },
      { trigger: '!ritual', description: 'Ritual diário, com chance de perder aura' },
      {
        trigger: '!mog',
        description: 'Desafia alguém para um duelo de aura',
        usages: [
          { syntax: '!mog @usuario', description: 'Envia o desafio' },
          { syntax: '!mog aceitar', description: 'Aceita o desafio recebido' },
        ],
      },
      { trigger: '!mognow', description: 'Duelo relâmpago aberto para quem responder primeiro' },
      {
        trigger: '!respeito',
        description: 'Presta respeito a alguém',
        usages: [{ syntax: '!respeito @usuario', description: 'Marque a pessoa' }],
      },
      {
        trigger: '!elogiar',
        description: 'Elogia alguém e transfere um pouco de aura',
        usages: [{ syntax: '!elogiar @usuario', description: 'Marque a pessoa' }],
      },
      {
        trigger: '!provocar',
        description: 'Provoca alguém e arrisca a própria aura',
        usages: [{ syntax: '!provocar @usuario', description: 'Marque a pessoa' }],
      },
      { trigger: '!elogiados', description: 'Lista quem você já elogiou' },
    ],
  },
  handle,
};
