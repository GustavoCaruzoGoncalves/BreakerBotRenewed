...
import type { EnrichedUser } from '../../services/level.js';
...
function formatUserInfo(
  ...
  if (showDailyDetails && info.dailyBonusMultiplier > 0) {
    const expiry = info.dailyBonusExpiry ? new Date(info.dailyBonusExpiry).getTime() : Date.now();
...
async function sendAdminResult(
  sock: WASocket,
  msg: BotMessage,
  sender: string,
  result: AdminResult,
...
const handle: CommandHandler = async (sock, msg) => {
  ...
  if (text === '!me' || text.startsWith('!me ')) {
    ...
    await sock.sendMessage(
      jid,
      { text: formatUserInfo(info, m.mentionText, isAdmin, true), mentions: m.mentions },
      { quoted: raw },
    );
    return true;
  }
  ...
  if (text.startsWith('!elos')) {
    ...
    level.RANKS.forEach((r) => {
      t += `${r.name} - Níveis ${r.minLevel} a ${r.maxLevel}\n`;
    });
  ...
  if (text.toLowerCase().startsWith('!setlevel')) {
    ...
    if (error) {
      await sock.sendMessage(jid, { text: error }, { quoted: raw });
      return true;
    }
    if (!targetId) {
      await sock.sendMessage(jid, { text: '📝 *Uso:* !setlevel @usuario nivel' }, { quoted: raw });
      return true;
    }
...
const levelCommand: Command = {
  meta: {
    category: 'Níveis',
    entries: [
      { trigger: '!me', description: 'Mostra o seu nível, XP, elo e prestígio' },
      {
        trigger: '!info',
        description: 'Mostra os dados de outra pessoa',
        usages: [{ syntax: '!info @usuario', description: 'Marque quem você quer consultar' }],
      },
      { trigger: '!elos', description: 'Lista todos os elos e o nível necessário para cada um' },
      { trigger: '!niveis', description: 'Explica como funcionam XP, níveis e prestígio' },
      { trigger: '!ranking', description: 'Top 10 do servidor por nível' },
      { trigger: '!prestigio', description: 'Troca um nível 10 por um ponto de prestígio' },
      { trigger: '!prestigioAll', description: 'Resgata de uma vez todos os prestígios disponíveis' },
      {
        trigger: '!setlevel',
        description: 'Define o nível de alguém',
        admin: true,
        usages: [{ syntax: '!setlevel @usuario <nível>', description: 'Sobrescreve o nível atual' }],
      },
      {
        trigger: '!resetsetlevel',
        description: 'Desfaz um !setlevel e devolve o nível original',
        admin: true,
        usages: [{ syntax: '!resetsetlevel @usuario', description: 'Marque quem será revertido' }],
      },
    ],
  },
  handle,
